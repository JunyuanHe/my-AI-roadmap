...
min_freq = 0  # you can adjust this
...
# Initialize random embedding matrix for your vocab
embedding_matrix = np.random.uniform(
    -0.1, 0.1, (len(vocab), embedding_dim)
).astype(np.float32)

for word, idx in vocab.items():
    # Encode word using Hugging Face tokenizer
    tokens = tokenizer.encode(word, add_special_tokens=False)
    
    if len(tokens) == 1:
        # Single token exists in GPT-2 vocab
        embedding_matrix[idx] = hf_embeddings[tokens[0]].detach().cpu().numpy()
    else:
        # Multi-token word: average subword embeddings
        sub_embs = hf_embeddings[tokens].detach().cpu().numpy()
        embedding_matrix[idx] = sub_embs.mean(axis=0)
...
embedding_weights = torch.from_numpy(embedding_matrix)
